...
def test_voice_endpoint():
    url = "http://localhost:8001/api/analyze-voice"
    ...
    try:
        health_response = requests.get("http://localhost:8001/api/session", timeout=5)
        print(f"Session check: {health_response.status_code}")
